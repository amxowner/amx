...
echo "compiling amx core..."
find src/java/amx/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "amx core class files compiled successfully"

echo "compiling amx desktop..."
find src/java/amxdesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "amx desktop class files compiled successfully"
else
    echo "if javafx is not supported, amx desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
