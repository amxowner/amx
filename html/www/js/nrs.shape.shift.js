...
var NRS = (function(NRS, $) {
    ...
    var coinToPair = function (op, coin) {
        return (op == "buy") ? "AMX_" + coin : coin + "_AMX";
    };

    var pairToCoin = function (pair) {
        if (pair.indexOf("AMX_") == 0) {
            return pair.substring("AMX_".length);
        }
        if (pair.indexOf("_AMX") == pair.length - "_AMX".length) {
            return pair.substring(0, pair.indexOf("_AMX"));
        }
    ...
    var renderExchangeTable = function (op) {
        ...
        async.series(tasks, function (err, results) {
            var table = $("#p_shape_shift_" + op + "_amx");
            if (err) {
        ...
        apiCall('recenttx/50', {}, 'GET', function (data) {
            ...
            if (data) {
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (String(transaction.curIn).escapeHTML() != "AMX" && String(transaction.curOut).escapeHTML() != "AMX") {
                        continue;
                    }
                    rows += "<tr>";
    ...
    function renderAmxLimit() {
        apiCall('limit/amx_btc', {}, 'GET', function (data) {
            NRS.logConsole("limit1 " + data.limit);
            if (data.limit) {
                $('#shape_shift_status').html('ok');
                $('#shape_shift_amx_avail').html(String(data.limit).escapeHTML());
            }
        });
    }
        ...
        apiCall('getcoins', {}, 'GET', function (data) {
            ...
            for (var i = 0; i < inputFields.length; i++) {
                ...
                $.each(data, function (code, coin) {
                    if (code != 'AMX' && coin['status'] == 'available') {
    ...
    $("#deposit_coin_modal").on("show.bs.modal", function (e) {
        ...
        var agentEmail= "admin@amaxcoin.cc";
        var coin = invoker.attr("coin-type");
        var title = invoker.text();
        $("#deposit_coin_modal_title").html(title);
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: coin, agentEmail: "amx_agent@email.com" }));
    ...
    NRS.pages.exchange = function() {
        ...
        //var coin = 'BTC';
        //var agentEmail = 'amx_agent@email.com';
        //$("#exchange_deposit_mail").html($.t("exchange_deposit_mail", { coin: "BTC", agentEmail: "amx_agent@email.com" }));
        loadDepositCoins();
    ...
    NRS.getFundAccountLink = function() {
        //return "<div class='callout callout-danger'>" +
            ...
            //"data-pair='BTC_AMX'>" + $.t("fund_account_message") + "</a>";
    ...
    $("#m_shape_shift_buy_modal").on("show.bs.modal", function (e) {
        ...
        $("#m_shape_shift_buy_title").html($.t("exchange_amx_to_coin_shift", { coin: coin }));
        $("#m_shape_shift_buy_min").val(invoker.data("min"));
        $("#m_shape_shift_buy_min_coin").html("AMX");
        $("#m_shape_shift_buy_max").val(invoker.data("max"));
        $("#m_shape_shift_buy_max_coin").html("AMX");
        $("#m_shape_shift_buy_rate").val(invoker.data("rate"));
        $("#m_shape_shift_buy_rate_text").html("AMX/" + coin);
    ...
    $("#m_send_amount_buy_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_buy_title").html($.t("exchange_amx_to_coin_send_amount", { coin: coin }));
        $("#m_send_amount_buy_withdrawal_amount_coin").html(coin);
        $("#m_send_amount_buy_rate_text").html("AMX/" + coin);
        ...
        apiCall('sendamount', {
            ...
            try {
                ...
                if (amount != data.success.withdrawalAmount) {
                    NRS.showModalError("amount returned from AMX exchange " + data.success.withdrawalAmount +
                    " differs from requested amount " + amount, modal);
                    return;
                }
                if (withdrawal != data.success.withdrawal) {
                    NRS.showModalError("withdrawal address returned from AMX exchange " + data.success.withdrawal +
                ...
                // add 1 AMX fee to make sure the net amount is what requested by shape shift
    ...
    $("#m_shape_shift_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_shape_shift_sell_title").html($.t("exchange_coin_to_amx_shift", { coin: coin }));
        ...
        async.waterfall([
            ...
            function(callback) {
                ...
                $("#m_shape_shift_sell_rate_text").html(coin + "/AMX");
                $("#m_shape_shift_sell_fee").val(data.fee);
                $("#m_shape_shift_sell_fee_coin").html("AMX");
    ...
    $("#m_send_amount_sell_modal").on("show.bs.modal", function (e) {
        ...
        var coin = pairToCoin(pair);
        NRS.logConsole("modal invoked pair " + pair + " coin " + coin);
        $("#m_send_amount_sell_title").html($.t("exchange_coin_to_amx_send_amount", { coin: coin }));
        $("#m_send_amount_sell_rate_text").html("AMX/" + coin);
        $("#m_send_amount_sell_fee_coin").html("AMX");
        $("#m_send_amount_sell_withdrawal_amount_coin").html("AMX");
