...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.initAdvancedModalFormValues = function($modal) {
		...
		context = {
			...
			inputName: "phasingQuorumAMX",
			addonText: "AMX",
			addonI18n: "amx_unit"
		};
		$elems = NRS.initModalUIElement($modal, '.approve_transaction_amount_amx', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		...
		context['inputName'] = 'phasingMinBalanceAMX';
		context['addonText'] = 'AMX';
		context['addonI18n'] = 'amx_unit';
		$elems = NRS.initModalUIElement($modal, '.approve_min_balance_amx', 'simple_input_with_addon_modal_ui_element', context);
		$elems.find('input').prop("disabled", true);
		$elems.hide();
