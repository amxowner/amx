    ...
    NRS.sendRequest = function (requestType, data, callback, options) {
        ...
        try {
            var winFields = [
                ...
                ["priceAMX", "priceNQT"],
                ["refundAMX", "refundNQT"],
                ["discountAMX", "discountNQT"],
                ["phasingQuorumAMX", "phasingQuorum"],
                ["phasingMinBalanceAMX", "phasingMinBalance"],
                ["controlQuorumAMX", "controlQuorum"],
                ["controlMinBalanceAMX", "controlMinBalance"],
                ["controlMaxFeesAMX", "controlMaxFees"],
                ["minBalanceAMX", "minBalance"],
                ["shufflingAmountAMX", "amount"],
                ["monitorAmountAMX", "amount"],
                ["monitorThresholdAMX", "threshold"]
            ];
            ...
            if (maxFees > 0 && new BigInteger(data.feeNQT).compareTo(new BigInteger(phasingControl.maxFees)) > 0) {
                callback({
                    ...
                    "errorDescription": $.t("error_fee_exceeds_max_account_control_fee", {
                        "maxFee": NRS.convertToAMX(phasingControl.maxFees)
                    })
                });
                return;
            }
        ...
        if (requestType == "getAccountId") {
            ...
            var amxAddress = new AmxAddress();
            var accountRS = "";
            if (amxAddress.set(accountId)) {
                accountRS = amxAddress.toString();
            }
            callback({
                "account": accountId,
                "accountRS": accountRS
