...
var NRS = (function(NRS, $) {
			...
			if (contacts && contacts.length) {
				...
				$.each(contacts, function (index, contact) {
					...
					rows += "<tr>" +
						...
						"<a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>" + $.t("send_amx") + "</a>&nbsp;" +
	...
	NRS.forms.addContact = function($modal) {
		...
		if (/^\d+$/.test(data.name) || /^AMX\-/i.test(data.name)) {
			return {
		...
		var address;
		if (/^AMX\-/i.test(data.account_id)) {
			data.account_rs = data.account_id;
			address = new AmxAddress();
	...
	$("#update_contact_modal").on("show.bs.modal", function(e) {
		...
		if (!contactId && NRS.selectedContext) {
			...
			var dbKey = (/^AMX\-/i.test(accountId) ? "accountRS" : "account");
			var dbQuery = {};
