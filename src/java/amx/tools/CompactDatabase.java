 ...
 *
 *   java -cp "classes:lib/*:conf" amx.tools.CompactDatabase
 *
 * To run the database compact tool on Windows:
 *
 *   java -cp "classes;lib/*;conf" -Damx.runtime.mode=desktop amx.tools.CompactDatabase
 */
public class CompactDatabase {
    ...
    public static void main(String[] args) {
        //
        // Initialize Amx properties and logging
        //
        Logger.init();
        //
    ...
    private static int compactDatabase() {
        ...
        //
        String dbPrefix = Constants.isTestnet ? "amx.testDb" : "amx.db";
        String dbType = Amx.getStringProperty(dbPrefix + "Type");
        if (!"h2".equals(dbType)) {
            Logger.logErrorMessage("Database type must be 'h2'");
            return 1;
        }
        String dbUrl = Amx.getStringProperty(dbPrefix + "Url");
        if (dbUrl == null) {
            String dbPath = Amx.getDbDir(Amx.getStringProperty(dbPrefix + "Dir"));
            dbUrl = String.format("jdbc:%s:%s", dbType, dbPath);
        }
        String dbParams = Amx.getStringProperty(dbPrefix + "Params");
        dbUrl += ";" + dbParams;
        if (!dbUrl.contains("MV_STORE=")) {
            dbUrl += ";MV_STORE=FALSE";
        }
        String dbUsername = Amx.getStringProperty(dbPrefix + "Username", "sa");
        String dbPassword = Amx.getStringProperty(dbPrefix + "Password", "sa", true);
        //
        ...
        File dbFile = new File(dbDir, "amx.h2.db");
        if (!dbFile.exists()) {
            dbFile = new File(dbDir, "amx.mv.db");
            if (!dbFile.exists()) {
