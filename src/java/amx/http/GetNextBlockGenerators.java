...
public final class GetNextBlockGenerators extends APIServlet.APIRequestHandler {
    ...
    protected JSONStreamAware processRequest(HttpServletRequest req) {
        ...
        if (block == null) {
            curBlock = Amx.getBlockchain().getLastBlock();
        } else {
            try {
                curBlock = Amx.getBlockchain().getBlock(Convert.parseUnsignedLong(block));
                if (curBlock == null) {
        ...
        */
        ...
        Block curBlock = Amx.getBlockchain().getLastBlock();
