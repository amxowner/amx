...
import static amx.http.JSONResponses.FEATURE_NOT_AVAILABLE;
import static amx.http.JSONResponses.INCORRECT_DEADLINE;
import static amx.http.JSONResponses.INCORRECT_EC_BLOCK;
import static amx.http.JSONResponses.INCORRECT_LINKED_FULL_HASH;
import static amx.http.JSONResponses.INCORRECT_WHITELIST;
import static amx.http.JSONResponses.MISSING_DEADLINE;
import static amx.http.JSONResponses.MISSING_SECRET_PHRASE;
import static amx.http.JSONResponses.NOT_ENOUGH_FUNDS;
...
abstract class CreateTransaction extends APIServlet.APIRequestHandler {
    ...
    private Appendix.Phasing parsePhasing(HttpServletRequest req) throws ParameterException {
        int finishHeight = ParameterParser.getInt(req, "phasingFinishHeight",
                Amx.getBlockchain().getHeight() + 1,
                Amx.getBlockchain().getHeight() + Constants.MAX_PHASING_DURATION + 1,
                true);
    ...
    final JSONStreamAware createTransaction(HttpServletRequest req, Account senderAccount, long recipientId,
                                            long amountNQT, Attachment attachment) throws AmxException {
        ...
        //Logger.logMessage("ecBlockId="+ecBlockId+" | ecBlockHeight="+ecBlockHeight+" | Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
        //ecBlockId = new BigInteger("429943628837330866").longValue();
        if (ecBlockId != 0 && ecBlockId != Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight)) {
        	//suse 10/03/2017
        	JSONObject response = new JSONObject();
            response.put("errorCode", 4);
            response.put("errorDescription", "Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight));
            return JSON.prepare(response);
        	//JSONStreamAware msg = incorrect("ecBlockId", "ecBlockId does not match the block id at ecBlockHeight");
        	//var message = JSONStreamAware (incorrect"Incorrect ecBlockId="+ecBlockId+" does not match the block id at ecBlockHeight="+ecBlockHeight+" - Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight)="+Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight); 
        	//return 
            //return INCORRECT_EC_BLOCK;
        }
        if (ecBlockId == 0 && ecBlockHeight > 0) {
            ecBlockId = Amx.getBlockchain().getBlockIdAtHeight(ecBlockHeight);
        }
        ...
        try {
            Transaction.Builder builder = Amx.newTransactionBuilder(publicKey, amountNQT, feeNQT,
            ...
            } catch (AmxException.NotYetEncryptedException ignore) {}
            ...
            if (broadcast) {
                Amx.getTransactionProcessor().broadcast(transaction);
                response.put("broadcasted", true);
            } else {
                transaction.validate();
                response.put("broadcasted", false);
            }
        } catch (AmxException.NotYetEnabledException e) {
            return FEATURE_NOT_AVAILABLE;
        } catch (AmxException.InsufficientBalanceException e) {
            throw e;
