...
import amx.AccountRestrictions;
import amx.Alias;
import amx.Appendix;
import amx.Asset;
import amx.AssetDelete;
import amx.AssetDividend;
import amx.AssetTransfer;
import amx.Attachment;
import amx.Block;
import amx.Constants;
import amx.Currency;
import amx.CurrencyExchangeOffer;
import amx.CurrencyFounder;
import amx.CurrencyTransfer;
import amx.CurrencyType;
import amx.DigitalGoodsStore;
import amx.Exchange;
import amx.ExchangeRequest;
import amx.FundingMonitor;
import amx.Generator;
import amx.HoldingType;
import amx.MonetarySystem;
import amx.Amx;
import amx.Order;
import amx.PhasingPoll;
import amx.PhasingVote;
import amx.Poll;
import amx.PrunableMessage;
import amx.Shuffler;
import amx.Shuffling;
import amx.ShufflingParticipant;
...
public final class JSONData {
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance) {
        return accountBalance(account, includeEffectiveBalance, Amx.getBlockchain().getHeight());
    }
    ...
    static JSONObject accountBalance(Account account, boolean includeEffectiveBalance, int height) {
        ...
        if (account == null) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAMX", "0");
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAMX", account.getEffectiveBalanceAMX(height));
    ...
    static JSONObject lessor(Account account, boolean includeEffectiveBalance) {
        ...
        if (accountLease.getCurrentLesseeId() != 0) {
            ...
            if (includeEffectiveBalance) {
                json.put("effectiveBalanceAMX", String.valueOf(account.getGuaranteedBalanceNQT() / Constants.ONE_AMX));
            }
        }
    ...
    static JSONObject shuffling(Shuffling shuffling, boolean includeHoldingInfo) {
        ...
        if (includeHoldingInfo && shuffling.getHoldingType() != HoldingType.AMX) {
        ...
        if (includeExecutedPhased) {
            ...
            try (DbIterator<PhasingPoll.PhasingPollResult> phasingPollResults = PhasingPoll.getApproved(block.getHeight())) {
                for (PhasingPoll.PhasingPollResult phasingPollResult : phasingPollResults) {
                    ...
                    if (includeTransactions) {
                        phasedTransactions.add(transaction(Amx.getBlockchain().getTransaction(phasedTransactionId)));
                    } else {
    ...
    static JSONObject pollResults(Poll poll, List<Poll.OptionResult> results, VoteWeighting voteWeighting) {
        ...
        if (voteWeighting.getMinBalanceModel() == VoteWeighting.MinBalanceModel.ASSET) {
            ...
            if (currency != null) {
                ...
                Transaction currencyIssuance = Amx.getBlockchain().getTransaction(voteWeighting.getHoldingId());
    ...
    static JSONObject transaction(Transaction transaction, Filter<Appendix> filter) {
        ...
        json.put("confirmations", Amx.getBlockchain().getHeight() - transaction.getHeight());
    ...
    private static void putExpectedTransaction(JSONObject json, Transaction transaction) {
        json.put("height", Amx.getBlockchain().getHeight() + 1);
        json.put("phased", transaction.getPhasing() != null);
    ...
    static void ledgerEntry(JSONObject json, LedgerEntry entry, boolean includeTransactions, boolean includeHoldingInfo) {
        ...
        if (includeTransactions && entry.getEvent().isTransaction()) {
            Transaction transaction = Amx.getBlockchain().getTransaction(entry.getEventId());
