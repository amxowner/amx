...
import amx.AccountLedger.LedgerEntry;
import amx.Block;
import amx.BlockchainProcessor;
import amx.Db;
import amx.Amx;
import amx.Transaction;
import amx.TransactionProcessor;
 ...
 * has not been received within amx.apiEventTimeout seconds.
 *
 * The maximum number of event users is specified by amx.apiMaxEventUsers.
 */
class EventListener implements Runnable, AsyncListener, TransactionalDb.TransactionCallback {

    /** Maximum event users */
    static final int maxEventUsers = Amx.getIntProperty("amx.apiMaxEventUsers");

    /** Event registration timeout (seconds) */
    static final int eventTimeout = Math.max(Amx.getIntProperty("amx.apiEventTimeout"), 15);

    /** Blockchain processor */
    static final BlockchainProcessor blockchainProcessor = Amx.getBlockchainProcessor();

    /** Transaction processor */
    static final TransactionProcessor transactionProcessor = Amx.getTransactionProcessor();
    ...
    /** Amx event listeners */
    private final List<AmxEventListener> amxEventListeners = new ArrayList<>();
     ...
     *
     * Amx event listeners will be added for the specified events
     *
     * @param   eventRegistrations      List of Amx event registrations
     ...
     *
     * @param   eventRegistrations      Amx event registrations
     * @throws  EventListenerException  Invalid Amx event
     */
    void addEvents(List<EventRegistration> eventRegistrations) throws EventListenerException {
        ...
        try {
            ...
            for (EventRegistration event : eventRegistrations) {
                ...
                Iterator<AmxEventListener> it = amxEventListeners.iterator();
                while (it.hasNext()) {
                    AmxEventListener listener = it.next();
                ...
                if (addListener) {
                    AmxEventListener listener = new AmxEventListener(event);
                    listener.addListener();
                    amxEventListeners.add(listener);
                }
            }
        } finally {
            lock.unlock();
    ...
    void deactivateListener() {
        ...
        try {
            ...
            //
            amxEventListeners.forEach(AmxEventListener::removeListener);
        } finally {
            lock.unlock();
        }
     ...
     * Amx event listener
     */
    private class AmxEventListener {

        /** Event handler */
        private final AmxEventHandler eventHandler;

        /**
         * Create the Amx event listener
         *
         * @param   eventRegistration           Event registration
         * @throws  EventListenerException      Invalid event
         */
        public AmxEventListener(EventRegistration eventRegistration) throws EventListenerException {
        ...
        /**
         * Add the Amx listener for this event
         */
        ...
        /**
         * Remove the Amx listener for this event
         */
        ...
        /**
         * Return the hash code for this Amx event listener
         *
         * @return                  Hash code
         */
        @Override
        ...
        /**
         * Check if two Amx events listeners are equal
         *
        ...
        public boolean equals(Object obj) {
            return (obj != null && (obj instanceof AmxEventListener) &&
                    eventHandler.equals(((AmxEventListener)obj).eventHandler));
        }

        /**
         * Amx listener event handler
         */
        private abstract class AmxEventHandler {
            ...
            /**
             * Create the Amx event handler
             *
             * @param   eventRegistration   Event registration
             */
            public AmxEventHandler(EventRegistration eventRegistration) {
            ...
            @Override
            public boolean equals(Object obj) {
                return (obj != null && (obj instanceof AmxEventHandler) &&
                                        owner == ((AmxEventHandler)obj).owner &&
                                        accountId == ((AmxEventHandler)obj).accountId &&
                                        event == ((AmxEventHandler)obj).event);
            }
        }
         ...
         * Peer event handler
         */
        private class PeerEventHandler extends AmxEventHandler implements Listener<Peer> {
         ...
         */
        private class BlockEventHandler extends AmxEventHandler implements Listener<Block> {
         ...
         */
        private class TransactionEventHandler extends AmxEventHandler implements Listener<List<? extends Transaction>> {
         ...
         */
        private class LedgerEventHandler extends AmxEventHandler implements Listener<LedgerEntry> {
    ...
    static class EventRegistration {
         ...
         *
         * @param   event           Amx listener event
        ...
        /**
         * Return the Amx listener event
         *
         * @return                  Amx listener event
         */
        public Enum<? extends Enum> getEvent() {
            return event;
        }
