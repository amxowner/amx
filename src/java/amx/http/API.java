...
import static amx.http.JSONResponses.INCORRECT_ADMIN_PASSWORD;
import static amx.http.JSONResponses.NO_PASSWORD_IN_CONFIG;
import static amx.http.JSONResponses.LOCKED_ADMIN_PASSWORD;
...
public final class API {
    ...
    public static final String adminPassword = Amx.getStringProperty("amx.adminPassword", "", true);
    static final boolean disableAdminPassword;
    static final int maxRecords = Amx.getIntProperty("amx.maxAPIRecords");
    static final boolean enableAPIUPnP = Amx.getBooleanProperty("amx.enableAPIUPnP");
    public static final int apiServerIdleTimeout = Amx.getIntProperty("amx.apiServerIdleTimeout");
    public static final boolean apiServerCORS = Amx.getBooleanProperty("amx.apiServerCORS");
    ...
    static {
        List<String> disabled = new ArrayList<>(Amx.getStringListProperty("amx.disabledAPIs"));
        Collections.sort(disabled);
        disabledAPIs = Collections.unmodifiableList(disabled);
        disabled = Amx.getStringListProperty("amx.disabledAPITags");
        Collections.sort(disabled);
        ...
        List<String> allowedBotHostsList = Amx.getStringListProperty("amx.allowedBotHosts");
        ...
        boolean enableAPIServer = Amx.getBooleanProperty("amx.enableAPIServer");
        if (enableAPIServer) {
            final int port = Constants.isTestnet ? TESTNET_API_PORT : Amx.getIntProperty("amx.apiServerPort");
            final int sslPort = Constants.isTestnet ? TESTNET_API_SSLPORT : Amx.getIntProperty("amx.apiServerSSLPort");
            final String host = Amx.getStringProperty("amx.apiServerHost");
            disableAdminPassword = Amx.getBooleanProperty("amx.disableAdminPassword") || ("127.0.0.1".equals(host) && adminPassword.isEmpty());

            apiServer = new Server();
            ServerConnector connector;
            boolean enableSSL = Amx.getBooleanProperty("amx.apiSSL");
            //
            ...
            if (enableSSL) {
                ...
                String keyStorePath = Paths.get(Amx.getUserHomeDir()).resolve(Paths.get(Amx.getStringProperty("amx.keyStorePath"))).toString();
                Logger.logInfoMessage("Using keystore: " + keyStorePath);
                sslContextFactory.setKeyStorePath(keyStorePath);
                sslContextFactory.setKeyStorePassword(Amx.getStringProperty("amx.keyStorePassword", null, true));
                ...
                sslContextFactory.setKeyStoreType(Amx.getStringProperty("amx.keyStoreType"));
                List<String> ciphers = Amx.getStringListProperty("amx.apiSSLCiphers");
                if (!ciphers.isEmpty()) {
            ...
            String apiResourceBase = Amx.getStringProperty("amx.apiResourceBase");
            if (apiResourceBase != null) {
                ...
                apiHandler.setWelcomeFiles(new String[]{Amx.getStringProperty("amx.apiWelcomeFile")});
            }

            String javadocResourceBase = Amx.getStringProperty("amx.javadocResourceBase");
            ...
            //ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/amx");
            ServletHolder servletHolder = apiHandler.addServlet(APIServlet.class, "/"+Constants.COIN_NAME.toLowerCase());
            servletHolder.getRegistration().setMultipartConfig(new MultipartConfigElement(
                    null, Math.max(Amx.getIntProperty("amx.maxUploadFileSize"), Constants.MAX_TAGGED_DATA_DATA_LENGTH), -1L, 0));

            //servletHolder = apiHandler.addServlet(APIProxyServlet.class, "/amx-proxy");
            ...
            if (!Amx.getBooleanProperty("amx.enableAPIServerGZIPFilter")) {
            	//gzipHandler.setExcludedPaths("/amx", "/amx-proxy");
                gzipHandler.setExcludedPaths("/"+Constants.COIN_NAME.toLowerCase(), "/"+Constants.COIN_NAME.toLowerCase()+"-proxy");
            }
            gzipHandler.setIncludedMethods("GET", "POST");
            gzipHandler.setMinGzipSize(amx.peer.Peers.MIN_COMPRESS_SIZE);
            apiHandler.setGzipHandler(gzipHandler);

            if (Amx.getBooleanProperty("amx.enableApiTestUI")) {
            ...
            if (Amx.getBooleanProperty("amx.apiFrameOptionsSameOrigin")) {
