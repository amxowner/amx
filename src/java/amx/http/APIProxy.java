...
public class APIProxy {
    ...
    static final boolean enableAPIProxy = Constants.isLightClient ||
            (Amx.getBooleanProperty("amx.enableAPIProxy") && API.openAPIPort == 0 && API.openAPISSLPort == 0);
    private static final int blacklistingPeriod = Amx.getIntProperty("amx.apiProxyBlacklistingPeriod") / 1000;
    static final String forcedServerURL = Amx.getStringProperty("amx.forceAPIProxyServerURL", "");
    ...
    static boolean isActivated() {
        return Constants.isLightClient || (enableAPIProxy && Amx.getBlockchainProcessor().isDownloading());
    }

    void blacklistHost(String host) {
        blacklistedPeers.put(host, Amx.getEpochTime() + blacklistingPeriod);
