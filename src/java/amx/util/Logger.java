...
/**
 * Handle logging for the Amx node server
 */
public final class Logger {
     ...
     * The amx/conf/logging-default.properties and amx/conf/logging.properties configuration
    ...
    static {
        ...
        System.setProperty("java.util.logging.manager", "amx.util.AmxLogManager");
        if (!(LogManager.getLogManager() instanceof AmxLogManager)) {
            System.setProperty("java.util.logging.manager",
                    (oldManager != null ? oldManager : "java.util.logging.LogManager"));
        }
        if (! Boolean.getBoolean("amx.doNotConfigureLogging")) {
            try {
                Properties loggingProperties = new Properties();
                Amx.loadProperties(loggingProperties, "logging-default.properties", true);
                Amx.loadProperties(loggingProperties, "logging.properties", false);
                Amx.updateLogFileHandler(loggingProperties);
        ...
        log = org.slf4j.LoggerFactory.getLogger(amx.Amx.class);
        enableStackTraces = Amx.getBooleanProperty("amx.enableStackTraces");
        enableLogTraceback = Amx.getBooleanProperty("amx.enableLogTraceback");
    ...
    public static void shutdown() {
        if (LogManager.getLogManager() instanceof AmxLogManager) {
            ((AmxLogManager) LogManager.getLogManager()).amxShutdown();
        }
    }
