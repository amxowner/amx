...
import static amx.user.JSONResponses.NOTIFY_OF_ACCEPTED_TRANSACTION;
...
public final class SendMoney extends UserServlet.UserRequestHandler {
    ...
    @Override
    JSONStreamAware processRequest(HttpServletRequest req, User user) throws AmxException.ValidationException, IOException {
        if (user.getSecretPhrase() == null) {
            return null;
        }

        String recipientValue = req.getParameter("recipient");
        String amountValue = req.getParameter("amountAMX");
        String feeValue = req.getParameter("feeAMX");
        ...
        try {
            ...
            amountNQT = Convert.parseAMX(amountValue.trim());
            feeNQT = Convert.parseAMX(feeValue.trim());
            ...
            response.put("amountAMX", amountValue);
            response.put("feeAMX", feeValue);
        ...
        } else if (feeNQT < Constants.ONE_AMX || feeNQT > Constants.MAX_BALANCE_NQT) {
        ...
        if (account == null || Math.addExact(amountNQT, feeNQT) > account.getUnconfirmedBalanceNQT()) {
            ...
            final Transaction transaction = Amx.newTransactionBuilder(user.getPublicKey(),
